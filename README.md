...
`pliscelle-mcp policy --disable` turns off on this device.
...
1. **Pair this device.** From the AIScelle tab in your Pli Scelle account, generate a pairing code,
   then run:
...
2. **Sign in.**
    ...
    This opens your browser to sign in and grant this device access to your mailbox. On a machine with
    no browser (a remote shell, a headless container), use the device flow instead:
