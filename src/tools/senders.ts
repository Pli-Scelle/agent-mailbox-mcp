...
export function registerSendersTool(server: McpServer): void {
	server.registerTool(
		'senders',
		{
			...
			description: `Lists authorized correspondents for this mailbox. \`ratifiedLocally\` reflects THIS device only: a correspondent the server reports as ratified may still be untrusted here until a human runs \`${cliCommand('ratify')}\`.`,
			inputSchema: {
