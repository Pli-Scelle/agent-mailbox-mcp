...
export class NoClientRegisteredError extends Error {
	constructor() {
		super(
			`No AIScelle device registration found on this machine. Run \`${cliCommand('pair --code <PAIRING_CODE>')}\` first (obtain a code from the AIScelle tab in your Pli Scelle account); it signs this device in automatically once paired.`,
		)
