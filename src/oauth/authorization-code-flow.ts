...
export async function runAuthorizationCodeLogin(params: RunAuthorizationCodeLoginParams): Promise<void> {
	...
	const authorizationUrl = buildAuthorizationUrl(params.configuration, {
		...
		// Not decoration, and not a UX choice: an authorization server drops
		// `offline_access` from the request unless consent is asked for
		// explicitly, which is what OIDC says to do and what `oidc-provider`
		// implements. Without this parameter the scope silently becomes
		// `mailbox:read mailbox:write`, the grant carries no offline access,
		// and no refresh token is ever issued. Nothing fails at that moment:
		// login succeeds, and this connector loses its access hours later
		// with no way to renew it. Measured against the real server on
		// 2026-08-30.
		prompt: 'consent',
	})
