...
export async function runDeviceLogin(params: RunDeviceLoginParams): Promise<void> {
	const deviceAuthorization = await initiateDeviceAuthorization(params.configuration, {
		...
		// Same reason as the browser flow: without an explicit consent
		// prompt the authorization server drops `offline_access` from the
		// request and issues no refresh token, silently. See
		// `authorization-code-flow.ts` for the measurement.
		prompt: 'consent',
	})
